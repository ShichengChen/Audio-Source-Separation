...
class Unet(nn.Module):
    def __init__(self, pad,sd, outc, rd, blocknum,device):
        self.sd = sd
        self.pad=pad
        super(Unet, self).__init__()
        if(pad>0):
            self.causal = nn.Conv1d(in_channels=1026, out_channels=rd, kernel_size=25, padding=0)
        else:
            self.causal = nn.Conv1d(in_channels=1026, out_channels=rd, kernel_size=25, padding=12)
        ...
        for i in range(self.blocknum):
            if (pad > 0):
                self.dcnn.append(nn.Conv1d(in_channels=rd, out_channels=rd, kernel_size=3, padding=0))
            else:
                self.dcnn.append(nn.Conv1d(in_channels=rd, out_channels=rd, kernel_size=3, padding=1))
