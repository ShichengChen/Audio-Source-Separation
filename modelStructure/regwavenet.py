...
class Wavenet(nn.Module):
    def __init__(self, pad, sd, qd, rd, dilations, device):
        ...
        self.causal0 = nn.Conv1d(in_channels=1, out_channels=rd, kernel_size=25, padding=12)
        # normal cnn
        self.pad = pad
        ...
        # normal cnn
        self.post2 = nn.Conv1d(in_channels=sd, out_channels=1, kernel_size=1)
        self.tanh = nn.Tanh()
    ...
    def forward(self, x):
        ...
        for i, d in enumerate(self.dilations):
            ...
            x += xinput
        x = self.post2(self.tanh(self.post1(F.leaky_relu(skip_connections))))
        return x